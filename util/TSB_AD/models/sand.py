...
class SAND():
	...
	"""
	def __init__(self,pattern_length,subsequence_length,k=6, normalize='z-norm'):
	...
	def _run(self,ts):
		...
		for index_name in range(len(self.clusters)):
			if self.nm_current_weight[index_name]> 0:
				## 260205 revised normalize by JJ
				# if self.normalize == 'zero-mean':
					# join = np.array([])
					# for i in range(0, len(ts)-self.pattern_length, self.pattern_length):
						# x_test = ts[i:i+self.pattern_length*2] - np.mean(ts[i:i+self.pattern_length*2])
						# join = np.append(join, stumpy.stump(x_test,self.pattern_length,self.clusters[index_name][0],ignore_trivial = False, normalize=False)[:,0])
				if self.normalize in ['euclidean', 'zero-mean']:
					join = stumpy.stump(ts,self.pattern_length,self.clusters[index_name][0],ignore_trivial = False, normalize=False)[:,0]
				else:
					join = stumpy.stump(ts,self.pattern_length,self.clusters[index_name][0],ignore_trivial = False, normalize=True)[:,0]
				join = np.array(join)
	...
	def _kshape_subsequence(self,initialization=True):
		...
		if len(all_subsequences) < self.k:
			k = min(self.k, len(all_subsequences))
			ks = KShape(n_clusters=k,verbose=False)
		else:
