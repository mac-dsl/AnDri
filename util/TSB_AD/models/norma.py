...
class NORMA():
    ...
    def __init__(self, pattern_length, nm_size, clustering='hierarchical',percentage_sel=0.4,overlapping_factor=1, sampling_division=10,number_of_cluster=10, normalize='z-norm'):
    ...
    def run(self, ts):
        ...
        if self.clustering == "hierarchical":
            recurrent_sequence,sequence_rec = extract_recurrent_sequences_random(
                ...
                # normalize=self.normalize)   ## add jj
...
def compute_score(ts, pattern_length, nms, scores_nms, normalize):
    ...
    for index_name in range(len(nms)):
        if normalize == 'zero-mean':
            join = np.array([])
            for i in range(0, len(ts)-pattern_length, pattern_length):
                x_test = ts[i:i+pattern_length*2] - np.mean(ts[i:i+pattern_length*2])
                join = np.append(join, stumpy.stump(x_test, pattern_length, nms[index_name], ignore_trivial =False, normalize=False)[:,0][:pattern_length])
        elif normalize == 'euclidean':
            join = stumpy.stump(ts,pattern_length,nms[index_name],ignore_trivial = False, normalize=False)[:,0]
        else:  # 'z-norm' or 'dtw'
            join = stumpy.stump(ts,pattern_length,nms[index_name],ignore_trivial = False, normalize=True)[:,0]
...
def extract_recurrent_sequences_random(ts, normalmodel_size, 
    ...
    for i,sr in enumerate(sequence_rec):
        # if normalize =='zero-mean':
            # recurrent_sequence[str(i)] = ts[(sr[0]):(sr[1])] - np.mean(ts[(sr[0]):(sr[1])])
        # else:    
...
def generate_dendrogram(recurrent_sequence, corr_method="pearson",linkage_method="complete",metric='euclidean'):
    ...
    correlation_matrix = np.nan_to_num(correlation_matrix, copy=True)
...
def choose_normalmodel(listcluster,recurrent_sequence, sequence_rec):
    ...
    for k in range(len(set(listcluster))):
        ...
        score_time.append(np.mean(i_seq_m))     # avg. of median
        min_max_index.append([np.min(i_seq_m), np.max(i_seq_m)]) # chk spread
