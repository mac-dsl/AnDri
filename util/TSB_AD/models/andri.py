...
class windowL:
    ...
    def enqueue(self, seq, idx):
        ...
        if num_active <=0:
            ...
            num_active += 1
            return_status.append(status_window.NON_STATIC)
...
class AnDri():
    ...
    def __init__(self,pattern_length, normalize='zero-mean', linkage_method='ward', th_reverse=5, 
        ...
        self.listcluster = []
        self.Z = []
        self.num_nm = np.array([])
        self.mem_nm = []
        self.dist_nm = []
        self.device_id = device_id
        self.plot_nm = plot_nm
        self.num_flip = 0
        self.count_flip = 0
        self.flips = []
        self.drift_points = np.array([])
        self.num_active_nm = np.array([])
        self.ahc_times = {'train':[], 'test':[]}
        self.runtime = {
            'train': [],
            'tr_cl_mgmt': [],
            'tr_score_comp': [],
    ...
    def __training(self, training_len, stepwise, min_size=0.025):
        ...
        # start_ahc = time.perf_counter()
        listcluster, Z, m_subseq, m_tau, m_nu, _, __cached__, d_ci, d_c_std, Ws, _ , self.num_flip, self.count_flip, self.flips= adaptive_ahc(
            seqs_n, linkage_method=self.linkage_method, th_reverse=self.th_reverse, kadj=self.kadj, delta_max=self.delta_max, eta=self.eta, max_W = self.max_W, min_size =min_size, rollback= self.rollback, metric=self.normalize, plot_nm=self.plot_nm)

        # end_ahc = time.perf_counter()
        # train_times['ahc'].append(end_ahc - start_time)
        ...
        for i in range(0, len(x_train), self.pattern_length):
            ...
            for j, nm in enumerate(self.NMs): 
                m_j, d_j = membership(nm.subseq, x_test_n, nm.tau, eta=1, metric=self.normalize)
            ...
            if cl_list[i] != -1:
                ...
                score = compute_seq_dist(self.NMs[nm_idx].subseq, x_test_n, metric=self.normalize)
                self.cl_s = np.append(self.cl_s, np.ones(self.pattern_length) * nm_idx)                

            else:
                ## Find candidate NMs here
                start_nm_mgmt = time.perf_counter()
                candidate_index = []
                W_candidate = self.pattern_length*self.delta_max
                n_W = len(cl_list)
                for k in range(i-1, max(0, i-W_candidate-1), -1):
                    if k< 0: break
                    if cl_list[k] != -1:
                        candidate_index.append(cl_list[k])
                        break
                for k in range(i+1, min(n_W, i+W_candidate+1)):
                    if cl_list[k] != -1:
                        candidate_index.append(cl_list[k])
                        break
                
                candidate_index = list(set(candidate_index))
                candidate_index = [cx for cx in candidate_index if cx != -1]
                
                if len(candidate_index) == 0:
                    lst = cl_list[max(0, i-self.pattern_length*self.delta_max):min(i+self.pattern_length*self.delta_max, len(cl_list))]
                    for x in lst:
                        if x != -1: break
                    if x == -1:
                        if max(0, i-self.pattern_length*self.delta_max) == 0:
                            # for k in cl_list[i+self.pattern_length*self.delta_max]:
                            for k in range(i+self.pattern_length*self.delta_max, i+3*self.pattern_length*self.delta_max, 1):
                                if cl_list[k] != -1: 
                                    candidate_index = [cl_list[k]]
                                    break
                        else:
                            for k in range(i-self.pattern_length*self.delta_max, 0, -1):
                                if cl_list[k] != -1:
                                    candidate_index = [cl_list[k]]
                                    break
                
                if len(candidate_index) == 0: candidate_index = [0]
                for j in candidate_index:
                    nm = self.NMs[int(j)]
                    score_cl.append(compute_seq_dist(nm.subseq, x_test_n, metric=self.normalize))

                score = np.min(score_cl)
                self.cl_s = np.append(self.cl_s, np.ones(self.pattern_length) * candidate_index[int(np.argmin(score_cl))])
                nm_idx = int(candidate_index[int(np.argmin(score_cl))])

                self.runtime['tr_cl_mgmt'].append(time.perf_counter() - start_nm_mgmt)

            selected_nm = self.NMs[nm_idx]
            selected_th = selected_nm.tau

            ## For training set, we only add one active normal patterns
            self.num_nm = np.append(self.num_nm, np.ones(self.pattern_length))
            ...
            if stepwise:
                ...
                rev_score = backward_anomaly_step(x_tests, self.pattern_length, self.NMs[nm_idx].subseq, self.normalize, self.device_id)
                ...
                if score > selected_th:
                    ...
                    tmp_score = tmp_score/(self.NMs[nm_idx].tau + 0.0001)
                    ...
                    if before_status:
                        ...
                        self.scores = np.append(self.scores, np.ones(self.pattern_length)*score/(self.NMs[nm_idx].tau + 0.0001))
                    before_status = False
                    
        end_score = time.perf_counter()
        self.runtime['tr_score_comp'].append(end_score - start_score)
    ...
    def fit(self, X, y=None, online=True, delta=0, training_len=None, stepwise=False, align=True, non_static =True, min_size=0.025, rollback=True):
        ...
        if online:
            ...
            ## init. active patterns
            self.drift_points = np.append(self.drift_points, self.seq_idx)
            self.num_active_nm = np.append(self.num_active_nm, len([nm for nm in self.NMs if nm.active == True]))

            start_score_test = time.perf_counter()
            while (self.seq_idx < len(X)):
                ...
                if len(test_seq) < self.pattern_length:
                    if stepwise:
                        ...
                        rev_score = backward_anomaly_step(x_tests, self.pattern_length, selected_nm.subseq, self.normalize, self.device_id)
                        self.scores = np.append(self.scores, rev_score[:self.pattern_length])
                    else:
                        if type(selected_nm) == list: selected_nm = selected_nm[0]
                        # score = np.linalg.norm(compute_diff_dist(test_seq_n, selected_nm.subseq))
                        score = compute_seq_dist(test_seq_n, selected_nm.subseq, metric=self.normalize)
                        self.scores = np.append(self.scores, np.ones(len(test_seq))*score/(selected_nm.tau + 0.0001))
                    print(f'Online scores len: {len(self.scores)}')
                    print('Done')
                    break
                
                start_mgmt = time.perf_counter()
                ...
                end_mgmt = time.perf_counter()
                self.runtime['nm_mgmt'].append(end_mgmt - start_mgmt)
                ...
                for j, nm in enumerate(movingWin.NM):
                    if nm.active:
                        ...
                        i_cl.append(j)
                        active_nm_num +=1
                self.num_nm = np.append(self.num_nm, active_nm_num*np.ones(self.pattern_length))

                if status_window.NON_STATIC in rsts and non_static:
                    score, th_idx, nm_idx = np.max(score_cl), np.argmax(score_cl), -1
                    self.cl_s = np.append(self.cl_s, nm_idx*np.ones(self.pattern_length))
                    if stepwise:
                        if self.cl_s[-self.pattern_length-1] == -1:
                            rev_score = np.ones(self.pattern_length) * score/(movingWin.NM[th_idx].tau + 0.0001)
                        else:
                            selected_nm = movingWin.NM[int(self.cl_s[int(-self.pattern_length)-1])]
                            rev_score = backward_anomaly_step(x_tests, self.pattern_length, selected_nm.subseq, self.normalize, device_id=self.device_id)
                        self.scores = np.append(self.scores, rev_score[:self.pattern_length])
                    else:
                        self.scores = np.append(self.scores, np.ones(self.pattern_length)*score/(movingWin.NM[th_idx].tau + 0.0001))
                else:
                    ...
                    if stepwise:
                        ...
                        if score > selected_th:
                            ...
                            tmp_score = tmp_score/(selected_th + 0.0001)
                            self.scores = np.append(self.scores, tmp_score)
                            before_status = True
                        else:
                            if before_status:
                                ...
                                self.scores = np.append(self.scores, np.ones(self.pattern_length)*score/(movingWin.NM[nm_idx].tau + 0.0001))
                            before_status=False
 
                self.seq_idx += self.pattern_length
                self.runtime['score_comp'].append(time.perf_counter() - end_mgmt)

            self.runtime['score_overall'].append(time.perf_counter() - start_score_test)
        ...
        if align:
            self.scores_rev = align_score(self.NMs, self.scores, self.cl_s[:len(self.scores)], self.pattern_length)
    ...
    def update_score(self, movingWin, curr_idx, nm_idx, score, stepwise, before_status):
        ...
        if stepwise:
            ...
            if score > selected_th:
                ...
                if before_status:
                    tmp_score = backward_anomaly(x_tests, self.pattern_length, selected_nm.subseq, self.normalize)
                    rev_score = tmp_score/(selected_th + 0.0001)
                else:
                    rev_score = np.ones(self.pattern_length)*score/(movingWin.NM[nm_idx].tau + 0.0001)
                before_status=False
    ...
    def revise_scores(self, movingWin, delta, nm_ids, stepwise, added=status_window.ADD, before_status=False):
        ...
        while (start_idx < self.seq_idx):
            ...
            if added == status_window.ACTIVE:
                ...
                new_dist = compute_seq_dist(movingWin.NM[nm_ids].subseq, self.ts[start_idx:start_idx+self.pattern_length], metric=self.normalize)
    ...
    def add_new_NM(self, t_seq, step, overlap, NMs, idx):
        ...
        listcluster, Z, m_subseq, m_tau, m_nu, _, _, d_ci, d_c_std, _, _, __cached__, __cached__, __cached__ = adaptive_ahc(
            win_seqs_n, linkage_method=self.linkage_method, th_reverse=10, kadj=self.kadj, delta_max = self.delta_max,eta=self.eta, max_W = self.max_W, min_size = 0.5, isTrain=False, NMs=NMs, rollback=self.rollback, metric=self.normalize, plot_nm=False)
        ...
        for m_s, m_tau, m_nu, m_m, m_std in zip(m_subseq, m_tau, m_nu, d_ci, d_c_std):
            ...
            for nm_i, nm in enumerate(NMs):
                ...
                temp_dist = compute_seq_dist(nm.subseq, m_s[:len(nm.subseq)], metric=self.normalize)
